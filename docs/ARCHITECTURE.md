...
Nidget/
  ...
  Features/
    ...
    Guide/
  Platform/AppShortcuts.swift AppIcon.swift PrivacyInfo.xcprivacy
...
- App icon (`Platform/AppIcon.swift`): the primary `AppIcon` asset carries its own light, dark and
  tinted appearances. One alternate icon per theme ships alongside it, named
  `ThemeIcon-<theme id with dots as dashes>`; `Preferences.themedAppIcon` opts into following the
  active theme and `RootView` applies it. `AppIcon.apply` must stay a no-op when the wanted icon is
  already showing, because iOS puts up an unsuppressable alert on every real change. All of the
  artwork is generated by `scripts/render-app-icons.pl` from the ThemeCatalog palettes.
